...
<ol>
文件被创建后，初始的权限如何设置？
需要全部用户都可以执行的指令，比如<code>ls</code>，它们的权限如何分配？
给一个文本文件分配了可执行权限会怎么样？
可不可以多个用户都登录<code>root</code>，然后只用<code>root</code>账户？
</ol>
...
adm 分组用于系统监控，比如<code>/var/log</code>中的部分日志就是 adm 分组。
...
尽可能少的<code>root</code>。如果一个用户需要<code>root</code>能力，那么应当进行权限包围——马上提升权限（比如 sudo），处理后马上释放权限。
