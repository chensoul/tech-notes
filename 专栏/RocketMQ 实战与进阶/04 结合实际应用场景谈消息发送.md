...
<strong>同步</strong>：客户端发起一次消息发送后会同步等待服务器的响应结果。
<strong>异步</strong>：客户端发起一下消息发起请求后不等待服务器响应结果而是立即返回，这样不会阻塞客户端子线程，当客户端收到服务端（Broker）的响应结果后会自动调用回调函数。
<strong>Oneway</strong>：客户端发起消息发送请求后并不会等待服务器的响应结果，也不会调用回调函数，即不关心消息的最终发送结果。
...
<code>producer#send</code> 方法时需要捕捉异常，常见的异常信息：<code>MQClientException(&quot;executor rejected &quot;, e)</code>。
