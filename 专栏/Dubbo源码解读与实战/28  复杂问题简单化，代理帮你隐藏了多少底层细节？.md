...
mInterfaces（Set<code>&lt;String&gt;</code> 类型）：代理类实现的接口。
mFields（List类型）：代理类中的字段。
mConstructors（List<code>&lt;String&gt;</code>类型）：代理类中全部构造方法的信息，其中包括构造方法的具体实现。
mMethods（List<code>&lt;String&gt;</code>类型）：代理类中全部方法的信息，其中包括方法的具体实现。
...
<ol>
加入 worked 集合（Set<code>&lt;String&gt;</code> 类型）中，用来判重。
将方法对应的 Method 对象添加到 methods 集合（List<code>&lt;Method&gt;</code> 类型）中。
