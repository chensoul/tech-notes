...
Database error
</code></pre><p>当错误发生时，调用log包打印错误。通过log包的caller功能，可以定位到log语句的位置，也就是定位到错误发生的位置。你使用这种方式来打印日志时，我有两个建议。</p>
...
</code></pre><p>可以看到<code>ErrCode</code>结构体包含了以下信息：</p>
int类型的业务码。
对应的HTTP Status Code。
暴露给外部用户的消息。
错误的参考文档。
<p>下面是一个具体的Coder示例：</p><pre><code>coder := &amp;ErrCode{
    C:    100001,
    HTTP: 200,
    Ext:  &quot;OK&quot;,
