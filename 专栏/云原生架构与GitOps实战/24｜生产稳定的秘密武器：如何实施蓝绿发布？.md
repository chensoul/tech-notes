...
在手动实施蓝绿发布的过程中，当流量切换到绿色环境时，如何将蓝色环境的副本数缩容至 0 ？
在上面的例子中，一旦更新 Rollout 对象的镜像版本，蓝绿发布过程就会自动进行。请你动手试试，如何使用 Rollout 对象的 autoPromotionEnabled 参数和 Argo Rollout kubectl 插件，实现手动控制蓝绿发布呢？（小提示：你可以参考<a href="https://argoproj.github.io/argo-rollouts/features/bluegreen/#example">这个链接</a>。）
