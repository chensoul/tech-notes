...
<p>但是如果需要调度的任务很多（例如上万个），这些任务的触发时间只相隔个几秒，这种通过一个线程一个一个检测任务的方式，很容易导致任务调度执行不精确。</p><p>为了解决这个问题，业界引入了时间轮算法，它的意思是引入时间轮，每一个轮代表一个时间刻度，如下图所示：</p><p><img src="https://static001.geekbang.org/resource/image/41/b1/41557194a39893fa2ba44087dcc7f5b1.jpg?wh=1560x912" alt="图片"></p><p>例如，图中每一个刻度代表1s。因为有8个格子，就代表8秒，也就是说，这个时间轮可以管理延迟调度时间在8s内的任务。如果想要增加延迟调度的时间范围，只要增加格子的数量即可。</p><p>具体怎么操作呢？</p>
