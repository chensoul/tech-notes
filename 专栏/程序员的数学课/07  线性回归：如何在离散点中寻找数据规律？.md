...
第 4 行，导入数据得到矩阵 <em><strong>X</strong></em>。为了还能求解出 b，我们需要对每个 xi 补充一个“1”；
第 5 行，导入数据得到真实值向量 <em><strong>ŷ</strong></em>。接下来，按照公式进行求解就可以了；
第 6 行，计算了 <em><strong>X</strong></em>T<em><strong>X</strong></em> 的结果；
第 7 行，对其求逆，得到了（<em><strong>X</strong></em>T<em><strong>X</strong></em>）-1；
第 8 行，再来对 <em><strong>X</strong></em>T 和 <strong>ŷ</strong> 计算矩阵乘法，得到最终的 <em><strong>w</strong></em>。
