...
Docker Engine需要使用命令行操作，主命令是 <code>docker</code>，后面再接各种子命令。
查看Docker的基本信息的命令是 <code>docker version</code> 和 <code>docker info</code> ，其他常用的命令有 <code>docker ps</code>、<code>docker pull</code>、<code>docker images</code>、<code>docker run</code>。
