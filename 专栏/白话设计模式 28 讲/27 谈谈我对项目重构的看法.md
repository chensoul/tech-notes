...
<strong>重复的代码太多</strong>，没有复用性；难于维护，需要修改时处处都得改。
<strong>代码的结构混乱</strong>，注释也不清晰；没有人能清楚地理解这段代码的含义。
<strong>程序没有拓展性</strong>，遇到新的变化，不能灵活的处理。
<strong>对象结构强耦合</strong>，业务逻辑太复杂，牵一发而动全身，维护时排查问题非常困难。
<strong>部分模块性能低</strong>，随着用户的增长，已无法满足响应速度的要求。
