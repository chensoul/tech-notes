...
![enter image description here](assets/a43e6950-966f-11e8-9f67-05ec09da262a.png)

按照上图所示的拓扑结构，如果节点 1 故障下线，那么节点 2 上的 A-S 将升主为 A-M，Redis 3 节点集群仍可用，如下图所示：

![enter image description here](assets/b04af9c0-966f-11e8-bee3-b1dbef72ca56.png)

特别说明：事实上，Redis 集群节点间是两两互通的，如下图所示，上面作为示意图，进行了适当简化。

![enter image description here](assets/c06bb2e0-966f-11e8-9c35-b59aad3fef8b.png)
...
![enter image description here](assets/3bd42b30-cd23-11e8-8e49-d7b1f250ebf6.jpeg)

##### **（3）创建必要启停脚本**

逐一手动拉起 Redis 进程较为麻烦，在此，我们可以编写简单的启停脚本完成 `redis-server` 进程的启停（`start.sh` 和 `stop.sh`）。

![enter image description here](assets/120e9810-9670-11e8-bee3-b1dbef72ca56.png)

##### **（4）简单测试**

至此，我们已经完成 Redis 集群创建的前期准备工作，在创建集群之前，我们可以简单测试一下，`redis-sever` 进程是否可以正常拉起。运行 `start.sh` 脚本，查看 `redis-server` 进程如下：

![enter image description here](assets/1b0eabd0-9670-11e8-9c35-b59aad3fef8b.png)

登录其中一个 Redis 实例的客户端（以 6379 为例），查看集群状态：很明显，以节点 6379 的视角来看，集群处于 Fail 状态，`clusterknownnodes:1` 表示集群中只有一个节点。

![enter image description here](assets/23e45430-9670-11e8-9c35-b59aad3fef8b.png)
...
![enter image description here](assets/49ae3fa0-9670-11e8-bee3-b1dbef72ca56.png)
...
![enter image description here](assets/614c8b30-9670-11e8-bd60-15398afc36e1.png)
...
![enter image description here](assets/9b307d70-9670-11e8-bd60-15398afc36e1.png)
...
![enter image description here](assets/a9e1a100-9670-11e8-bee3-b1dbef72ca56.png)

设置密码后，Lettuce 客户端访问 `redis-server` 就需要鉴权，增加一行代码即可，以单机模式为例：

![enter image description here](assets/bb32eae0-9670-11e8-9f67-05ec09da262a.png)
...
![enter image description here](assets/f1d17620-9670-11e8-bd60-15398afc36e1.png)
...
![enter image description here](assets/03a25040-9671-11e8-9c35-b59aad3fef8b.png)

**redis.h**

Redis 的客户端（redisClient）和服务端（redisServer）都需要适配，部分代码如下：

![enter image description here](assets/0bb767c0-9671-11e8-9f67-05ec09da262a.png)

![enter image description here](assets/19777490-9671-11e8-bd60-15398afc36e1.png)

**hiredis.h**

修改创建连接的原函数：

![enter image description here](assets/28862170-9671-11e8-9f67-05ec09da262a.png)

**anet.h**

定义 SSL 通信涉及的一些函数（实现在 anet.c 中）：

![enter image description here](assets/2e74d680-9671-11e8-9c35-b59aad3fef8b.png)
...
![enter image description here](assets/5e5c0580-9671-11e8-bee3-b1dbef72ca56.png)

#### 4.1 可靠性问题一

Redis 集群并不是将 `redis-server` 进程启动便可自行建立的。在各个节点启动 `redis-server` 进程后，形成的只是 6 个“孤立”的 Redis 节点而已，它们相互不知道对方的存在，拓扑结构如下：

![enter image description here](assets/6aa306e0-9671-11e8-bd60-15398afc36e1.png)
...
![enter image description here](assets/7a5b8850-9671-11e8-9c35-b59aad3fef8b.png)

#### 4.3 故障节点恢复

接续上一节，如果宕机的节点 1 经过修复重新上线，根据 Redis 集群原理，节点 1 上的 A-M 将意识到自己已经被替代，将降级为备，形成的集群拓扑结构如下：

![enter image description here](assets/81b34340-9671-11e8-9f67-05ec09da262a.png)
...
![enter image description here](assets/8e851f80-9671-11e8-bee3-b1dbef72ca56.png)
...
![enter image description here](assets/962f4030-9671-11e8-9c35-b59aad3fef8b.png)
...
![enter image description here](assets/a0b9eb90-9671-11e8-bee3-b1dbef72ca56.png)
...
![enter image description here](assets/a93a20f0-9671-11e8-bd60-15398afc36e1.png)
...
![enter image description here](assets/cab6ab40-9671-11e8-bee3-b1dbef72ca56.png)
...
![enter image description here](assets/5ac7ecd0-9672-11e8-bd60-15398afc36e1.png)
...
![enter image description here](assets/736e0b70-9672-11e8-bee3-b1dbef72ca56.png)
