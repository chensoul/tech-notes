...
Loki：核心组件，负责日志存储和处理查询。
Promtail：日志收集工具，负责收集 Pod 的日志并发送给 Loki。
Grafana：UI Dashboard，用于查询日志。
...
分组统计 1 分钟内的请求状态码数量。
统计 TP99 的请求耗时并分组。
<p>由于示例应用输出了 logfmt 和 JSON 两种格式的日志，所以在所有的例子中，我都会通过管道操作 “status=” 来匹配 logfmt 的日志，以便把它和 JSON 格式的日志区分开。此外，你可以先把 Grafana 的查询界面切换到<strong>Code 模式</strong>，这样就可以直接复制下面的语句来进行试验了。</p><h3>查询状态码不等于 200 的日志</h3><p>在生产环境下，查询返回状态码不等于 200 的场景是比较常见的，以示例应用为例，你可以通过下面的 LogQL 来查询。</p><pre><code class="language-yaml">{app="log-example"} |= `status=` | logfmt | status != `200`
